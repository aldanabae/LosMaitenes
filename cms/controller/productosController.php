...
class productosController extends Controller {
  ...
  public function listar($type = 1) {
    $data['title'] = 'Listando Productos';
    $data['tabla'] = $this->_tabla;
    $data['type'] = $type;

    $this->getBreadcrumb('Productos', 'listar');

    if (Validator::validateInteger('listar', true) == 1) {
      $opt = Validator::sanitizeInteger('accion', true);
      $ids = $_POST['selected'];

      ($opt == 4) ? $this->_model->deleteProductos($ids) : $this->_model->updateProductos($opt, $ids);
    }
  ...
  public function insertar() {
    ...
    if (Validator::validateInteger('actualizar', true) == 1) {
      ...
      if (empty($titulo)) {
        $data['alert'] = $this->alertMessage('danger', 'Debe ingresar el Nombre del Producto');
        $this->_view->assign('data', $data);
        $this->_view->render();
        exit;
      }

      $id = $this->_model->updateProducto($fields);
  ...
  public function editar($id) {
    ...
    if (Validator::validateInteger('actualizar', true) == 1) {
      ...
      $fields['ID'] = $id;
      $this->_model->updateProducto($fields);

      Url::redirect($this->base_url . 'listar');
    }

    $data[$this->_tabla] = $this->_model->getProducto($id);
