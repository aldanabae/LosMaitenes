...
class homeModel extends Model {
  ...
  public function getSeccion($url) {
    ...
    $query = "SELECT t1.*, t2.Archivo FROM " . $t1 . " t1 "
            . " LEFT JOIN " . $t2 . " t2 ON t1.PlantillaID=t2.ID "
            . " WHERE t1.URL='" . $url . "'";
    $row = $this->_db->query($query);
  ...
  public function getGaleriaFotos($id) {
    $result = array();
    $galeria = $this->where(array('table' => 'galerias', 'where' => "ID=" . $id));

    if ($galeria) {
      $result['info'] = $galeria;
      $result['fotos'] = $this->all(array('table' => 'archivo', 'field' => 'Titulo, Imagen', 'where' => "Tabla='galerias' AND TablaID='" . $id . "'", 'order' => 'Posicion'));
    }
  ...
  public function getSecciones() {
    return $this->all(array('table' => 'seccion', 'where' => "Publico='1'", 'order'=>'Posicion'));
  }
